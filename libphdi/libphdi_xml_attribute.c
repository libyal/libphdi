...
{
	static char *function = "libphdi_xml_attribute_initialize";
	...
	*attribute = memory_allocate_structure(
	...
	if( memory_set(
	     *attribute,
	     0,
	     sizeof( libphdi_xml_attribute_t ) ) == NULL )
	{
		libcerror_error_set(
		 error,
		 LIBCERROR_ERROR_DOMAIN_MEMORY,
...
int libphdi_xml_attribute_free(
     ...
     libcerror_error_t **error )
{
	static char *function = "libphdi_xml_attribute_free";
