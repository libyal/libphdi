...
{
	static char *function = "libphdi_block_tree_initialize";
	...
	*block_tree = memory_allocate_structure(
	...
	if( memory_set(
	     *block_tree,
	     0,
	     sizeof( libphdi_block_tree_t ) ) == NULL )
	{
		libcerror_error_set(
		 error,
		 LIBCERROR_ERROR_DOMAIN_MEMORY,
	...
	if( libphdi_block_tree_node_initialize(
...
int libphdi_block_tree_free(
     ...
     libcerror_error_t **error )
{
	static char *function = "libphdi_block_tree_free";
	int result            = 1;
     ...
     libcerror_error_t **error )
{
	...
	{
		if( libphdi_block_tree_node_get_sub_node_at_offset(
		     block_tree_node,
		     offset,
		     &block_tree_node,
		     error ) != 1 )
     ...
     libcerror_error_t **error )
{
	...
	while( safe_block_tree_node->is_leaf_node == 0 )
	{
		...
		if( block_tree_node == NULL )
		{
			...
			if( libphdi_block_tree_node_set_sub_node_at_offset(
			     safe_block_tree_node,
			     offset,
			     sub_block_tree_node,
			     error ) != 1 )
	...
	*leaf_block_tree_node = safe_block_tree_node;
	...
	if( libphdi_block_tree_node_set_leaf_value_at_offset(
	     safe_block_tree_node,
	     offset,
	     block_descriptor,
	     error ) != 1 )
