...
then
	docker run -t -v "${PWD}:/libphdi" ${DOCKERHUB_REPO}:${DOCKERHUB_TAG} sh -c "cd libphdi && .travis/script_docker.sh";
...
then
	...
	then
		...
		cp AUTHORS COPYING NEWS README ${PWD}/osx-pkg/usr/share/doc/libphdi;
