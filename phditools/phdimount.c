      ...
      FILE *stream )
{
	...
	fprintf( stream, "Use phdimount to mount the Parallels Hard Disk\n"
                         "image file\n\n" );

	fprintf( stream, "Usage: phdimount [ -X extended_options ] [ -hvV ]\n"
	                 "                 phdi_file mount_point\n\n" );

	fprintf( stream, "\tphdi_file:   the PHDI image file\n\n" );
	fprintf( stream, "\tmount_point: the directory to serve as mount point\n\n" );

	fprintf( stream, "\t-h:          shows this help\n" );
	fprintf( stream, "\t-v:          verbose output to stderr\n"
	                 "\t             phdimount will remain running in the foreground\n" );
...
static char *phdimount_fuse_path_prefix         = "/phdi";
static size_t phdimount_fuse_path_prefix_length = 5;

#if defined( HAVE_TIME )
time_t phdimount_timestamp                      = 0;
#endif

/* Opens a file or directory
 * Returns 0 if successful or a negative errno value otherwise
 */
int phdimount_fuse_open(
     const char *path,
     struct fuse_file_info *file_info )
{
	libcerror_error_t *error = NULL;
	static char *function    = "phdimount_fuse_open";
	...
	if( ( path_length <= phdimount_fuse_path_prefix_length )
	 || ( narrow_string_compare(
	       path,
	       phdimount_fuse_path_prefix,
	       phdimount_fuse_path_prefix_length ) != 0 ) )
	{
		libcerror_error_set(
		 &error,
		 LIBCERROR_ERROR_DOMAIN_ARGUMENTS,
...
{
	libcerror_error_t *error = NULL;
	static char *function    = "phdimount_fuse_read";
	size_t path_length       = 0;
	...
	if( ( path_length <= phdimount_fuse_path_prefix_length )
	 || ( path_length > ( phdimount_fuse_path_prefix_length + 3 ) )
	 || ( narrow_string_compare(
	       path,
	       phdimount_fuse_path_prefix,
	       phdimount_fuse_path_prefix_length ) != 0 ) )
	...
	string_index = (int) phdimount_fuse_path_prefix_length;
 ...
 */
int phdimount_fuse_set_stat_info(
...
{
	static char *function = "phdimount_fuse_set_stat_info";
	...
	if( use_mount_time != 0 )
	{
		if( phdimount_timestamp == 0 )
		{
			if( time(
			     &phdimount_timestamp ) == (time_t) -1 )
			{
				phdimount_timestamp = 0;
			}
		}
		stat_info->st_atime = phdimount_timestamp;
		stat_info->st_mtime = phdimount_timestamp;
		stat_info->st_ctime = phdimount_timestamp;
	}
#endif
	if( size != 0 )
	{
...
int phdimount_fuse_filldir(
     ...
     libcerror_error_t **error )
{
	static char *function   = "phdimount_fuse_filldir";
	...
	if( phdimount_fuse_set_stat_info(
	     stat_info,
	     media_size,
	     number_of_sub_items,
	     use_mount_time,
...
{
	char phdimount_fuse_path[ 10 ];

	libcerror_error_t *error = NULL;
	struct stat *stat_info   = NULL;
	static char *function    = "phdimount_fuse_readdir";
	size_t path_length       = 0;
	int input_file_index     = 0;
	...
	if( narrow_string_copy(
	     phdimount_fuse_path,
	     phdimount_fuse_path_prefix,
	     phdimount_fuse_path_prefix_length ) == NULL )
	{
		libcerror_error_set(
		 &error,
		 LIBCERROR_ERROR_DOMAIN_MEMORY,
	...
	for( input_file_index = 1;
	     ...
	     input_file_index++ )
	{
		string_index = phdimount_fuse_path_prefix_length;

		if( input_file_index >= 100 )
		{
			phdimount_fuse_path[ string_index++ ] = '0' + (char) ( input_file_index / 100 );
		}
		if( input_file_index >= 10 )
		{
			phdimount_fuse_path[ string_index++ ] = '0' + (char) ( input_file_index / 10 );
		}
		phdimount_fuse_path[ string_index++ ] = '0' + (char) ( input_file_index % 10 );
		phdimount_fuse_path[ string_index++ ] = 0;

		if( phdimount_fuse_filldir(
		     buffer,
		     filler,
		     &( phdimount_fuse_path[ 1 ] ),
		     string_index - 1,
		     stat_info,
		     phdimount_mount_handle,
		     input_file_index - 1,
 ...
 * Returns 0 if successful or a negative errno value otherwise
 */
int phdimount_fuse_getattr(
     const char *path,
     struct stat *stat_info )
{
	libcerror_error_t *error = NULL;
	static char *function    = "phdimount_fuse_getattr";
	size64_t media_size      = 0;
	size_t path_length       = 0;
	int input_file_index     = 0;
	int number_of_sub_items  = 0;
	...
	else if( ( path_length > phdimount_fuse_path_prefix_length )
	      && ( path_length <= ( phdimount_fuse_path_prefix_length + 3 ) ) )
	{
 ...
 */
void phdimount_fuse_destroy(
      void *private_data PHDITOOLS_ATTRIBUTE_UNUSED )
{
	libcerror_error_t *error = NULL;
	static char *function    = "phdimount_fuse_destroy";

	PHDITOOLS_UNREFERENCED_PARAMETER( private_data )
...
static wchar_t *phdimount_dokan_path_prefix      = L"\\PHDI";
static size_t phdimount_dokan_path_prefix_length = 5;

/* Opens a file or directory
 * Returns 0 if successful or a negative error code otherwise
 */
int __stdcall phdimount_dokan_CreateFile(
               const wchar_t *path,
               DWORD desired_access,
               DWORD share_mode PHDITOOLS_ATTRIBUTE_UNUSED,
               DWORD creation_disposition,
               DWORD attribute_flags PHDITOOLS_ATTRIBUTE_UNUSED,
               DOKAN_FILE_INFO *file_info )
{
	libcerror_error_t *error = NULL;
	static char *function    = "phdimount_dokan_CreateFile";
	size_t path_length       = 0;
	int result               = 0;

	PHDITOOLS_UNREFERENCED_PARAMETER( share_mode )
	PHDITOOLS_UNREFERENCED_PARAMETER( attribute_flags )
	...
	{
		if( ( path_length <= phdimount_dokan_path_prefix_length )
		 || ( path_length > ( phdimount_dokan_path_prefix_length + 3 ) )
		 || ( wide_string_compare(
		       path,
		       phdimount_dokan_path_prefix,
		       phdimount_dokan_path_prefix_length ) != 0 ) )
		{
			libcerror_error_set(
			 &error,
			 LIBCERROR_ERROR_DOMAIN_ARGUMENTS,
 ...
 * Returns 0 if successful or a negative error code otherwise
 */
int __stdcall phdimount_dokan_OpenDirectory(
               const wchar_t *path,
               DOKAN_FILE_INFO *file_info PHDITOOLS_ATTRIBUTE_UNUSED )
{
	libcerror_error_t *error = NULL;
	static char *function    = "phdimount_dokan_OpenDirectory";
	size_t path_length       = 0;
	int result               = 0;
 ...
 * Returns 0 if successful or a negative error code otherwise
 */
int __stdcall phdimount_dokan_CloseFile(
               const wchar_t *path,
               DOKAN_FILE_INFO *file_info PHDITOOLS_ATTRIBUTE_UNUSED )
{
	libcerror_error_t *error = NULL;
	static char *function    = "phdimount_dokan_CloseFile";
	int result               = 0;
 ...
 * Returns 0 if successful or a negative error code otherwise
 */
int __stdcall phdimount_dokan_ReadFile(
               const wchar_t *path,
               void *buffer,
...
{
	libcerror_error_t *error = NULL;
	static char *function    = "phdimount_dokan_ReadFile";
	size_t path_length       = 0;
	ssize_t read_count       = 0;
	int input_file_index     = 0;
	int result               = 0;
	...
	string_index = (int) phdimount_dokan_path_prefix_length;
 ...
 * Returns 1 if successful or -1 on error
 */
int phdimount_dokan_set_find_data(
     ...
     libcerror_error_t **error )
{
	static char *function = "phdimount_dokan_set_find_data";
...
int phdimount_dokan_filldir(
     ...
     libcerror_error_t **error )
{
	static char *function   = "phdimount_dokan_filldir";
	size64_t media_size     = 0;
	int number_of_sub_items = 0;
	...
	if( phdimount_dokan_set_find_data(
	     find_data,
	     media_size,
	     number_of_sub_items,
	     use_mount_time,
 ...
 * Returns 0 if successful or a negative error code otherwise
 */
int __stdcall phdimount_dokan_FindFiles(
               const wchar_t *path,
               PFillFindData fill_find_data,
               DOKAN_FILE_INFO *file_info )
{
	...
	wchar_t phdimount_dokan_path[ 10 ];

	libcerror_error_t *error = NULL;
	static char *function    = "phdimount_dokan_FindFiles";
	size64_t media_size      = 0;
	size_t path_length       = 0;
	int input_file_index     = 0;
	int number_of_inputs     = 0;
	...
	if( wide_string_copy(
	     phdimount_dokan_path,
	     phdimount_dokan_path_prefix,
	     phdimount_dokan_path_prefix_length ) == NULL )
	{
		libcerror_error_set(
		 &error,
		 LIBCERROR_ERROR_DOMAIN_MEMORY,
	...
	for( input_file_index = 1;
	     ...
	     input_file_index++ )
	{
		string_index = phdimount_dokan_path_prefix_length;

		if( input_file_index >= 100 )
		{
			phdimount_dokan_path[ string_index++ ] = (wchar_t) ( '0' + ( input_file_index / 100 ) );
		}
		if( input_file_index >= 10 )
		{
			phdimount_dokan_path[ string_index++ ] = (wchar_t) ( '0' + ( input_file_index / 10 ) );
		}
		phdimount_dokan_path[ string_index++ ] = (wchar_t) ( '0' + ( input_file_index % 10 ) );
		phdimount_dokan_path[ string_index++ ] = 0;

		if( phdimount_dokan_filldir(
		     fill_find_data,
		     file_info,
		     &( phdimount_dokan_path[ 1 ] ),
		     string_index - 1,
		     &find_data,
		     phdimount_mount_handle,
		     input_file_index - 1,
 ...
 * Returns 1 if successful or -1 on error
 */
int phdimount_dokan_set_file_information(
     ...
     libcerror_error_t **error )
{
	static char *function = "phdimount_dokan_set_file_information";
 ...
 * Returns 0 if successful or a negative error code otherwise
 */
int __stdcall phdimount_dokan_GetFileInformation(
               const wchar_t *path,
               BY_HANDLE_FILE_INFORMATION *file_information,
               DOKAN_FILE_INFO *file_info )
{
	libcerror_error_t *error = NULL;
	static char *function    = "phdimount_dokan_GetFileInformation";
	size64_t media_size      = 0;
	size_t path_length       = 0;
	int input_file_index     = 0;
	int number_of_sub_items  = 0;
	...
	if( phdimount_dokan_set_file_information(
	     file_information,
	     media_size,
	     number_of_sub_items,
	     use_mount_time,
 ...
 * Returns 0 if successful or a negative error code otherwise
 */
int __stdcall phdimount_dokan_GetVolumeInformation(
               wchar_t *volume_name,
...
{
	libcerror_error_t *error = NULL;
	static char *function    = "phdimount_dokan_GetVolumeInformation";
	int result               = 0;
 ...
 * Returns 0 if successful or a negative error code otherwise
 */
int __stdcall phdimount_dokan_Unmount(
               DOKAN_FILE_INFO *file_info PHDITOOLS_ATTRIBUTE_UNUSED )
{
	static char *function = "phdimount_dokan_Unmount";
...
#endif
{
	...
	phdimount_fuse_operations.open    = &phdimount_fuse_open;
	phdimount_fuse_operations.read    = &phdimount_fuse_read;
	phdimount_fuse_operations.readdir = &phdimount_fuse_readdir;
	phdimount_fuse_operations.getattr = &phdimount_fuse_getattr;
	phdimount_fuse_operations.destroy = &phdimount_fuse_destroy;
	...
	phdimount_dokan_options.Version     = 600;
...
*/
	...
	phdimount_dokan_operations.CreateFile           = &phdimount_dokan_CreateFile;
	phdimount_dokan_operations.OpenDirectory        = &phdimount_dokan_OpenDirectory;
	phdimount_dokan_operations.CreateDirectory      = NULL;
	phdimount_dokan_operations.Cleanup              = NULL;
	phdimount_dokan_operations.CloseFile            = &phdimount_dokan_CloseFile;
	phdimount_dokan_operations.ReadFile             = &phdimount_dokan_ReadFile;
	phdimount_dokan_operations.WriteFile            = NULL;
	phdimount_dokan_operations.FlushFileBuffers     = NULL;
	phdimount_dokan_operations.GetFileInformation   = &phdimount_dokan_GetFileInformation;
	phdimount_dokan_operations.FindFiles            = &phdimount_dokan_FindFiles;
	...
	phdimount_dokan_operations.GetVolumeInformation = &phdimount_dokan_GetVolumeInformation;
	phdimount_dokan_operations.Unmount              = &phdimount_dokan_Unmount;
