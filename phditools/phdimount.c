...
#endif
{
	const char *description               = \
		"Use phdimount to mount a Parallels Hard Disk image file.";

	phditools_option_t options[ ] = {
		{ 'h', NULL, "shows this help" },
		{ 'v', NULL, "verbose output to stderr, while phdimount will remain running in the foreground" },
		{ 'V', NULL, "print version" },
#if defined( HAVE_LIBFUSE ) || defined( HAVE_LIBFUSE3 ) || defined( HAVE_LIBOSXFUSE )
		{ 'X', "extended_options", "extended options to pass to sub system" },
#endif
		{ 0, "image", "a Parallels Hard Disk image file" },
	...
	int number_of_options                       = (int) ( sizeof( options ) / sizeof( phditools_option_t ) );
	int verbose                                 = 0;
	...
	struct fuse_args phdimount_fuse_arguments   = FUSE_ARGS_INIT(1, fuse_argv);
#else
	...
	if( phditools_getopt_get_options_string(
	     options,
	     number_of_options,
	     options_string,
	     32 ) != 1 )
	...
	if( option_extended_options != NULL )
	{
		...
		phdimount_fuse_arguments.argc = 0;
		phdimount_fuse_arguments.argv = NULL;
#endif
	...
	phdimount_dokan_options.SingleThread = TRUE;
#else
...
*/
	...
	fprintf(
	 stderr,
	 "No sub system to mount Parallels Hard Disk image (PHDI) format.\n" );
