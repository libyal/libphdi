...
TEST_EXECUTABLE="../phditools/phdiinfo";

if ! test -x "${TEST_EXECUTABLE}";
then
	TEST_EXECUTABLE="../phditools/phdiinfo.exe";
fi
...
TEST_PROFILE_DIRECTORY=$(get_test_profile_directory "input" "phdiinfo");
...
";
	...
	then
		...
		do
			run_test_on_input_file_with_options "${TEST_SET_DIRECTORY}" "phdiinfo" "with_stdout_reference" "${OPTION_SETS}" "${TEST_EXECUTABLE}" "${INPUT_FILE}";
			RESULT=$?;
		...
		do
			run_test_on_input_file_with_options "${TEST_SET_DIRECTORY}" "phdiinfo" "with_stdout_reference" "${OPTION_SETS}" "${TEST_EXECUTABLE}" "${INPUT_FILE}" "${OPTIONS}";
			RESULT=$?;
