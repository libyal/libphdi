...
def pyphdi_test_single_open_close_file(filename, mode):
...
def pyphdi_test_multi_open_close_file(filename, mode):
...
def pyphdi_test_single_open_close_file_object(filename, mode):
  ...
  try:
    ...
    phdi_file.open_file_object(file_object, mode)
    phdi_file.close()
...
def pyphdi_test_single_open_close_file_object_with_dereference(
    filename, mode):
...
def pyphdi_test_multi_open_close_file_object(filename, mode):
...
def main():
  ...
  if not pyphdi_test_single_open_close_file(options.source, "r"):
    return False

  if not pyphdi_test_single_open_close_file(None, "r"):
    return False

  if not pyphdi_test_single_open_close_file(options.source, "w"):
    return False

  if not pyphdi_test_multi_open_close_file(options.source, "r"):
    return False

  if not pyphdi_test_single_open_close_file_object(options.source, "r"):
    return False

  if not pyphdi_test_single_open_close_file_object_with_dereference(
      options.source, "r"):
    return False

  if not pyphdi_test_multi_open_close_file_object(
      options.source, "r"):
    return False
